...
void CustomSort::setSortFields(QStringList fields)
{
    ui->sort_label->setText("Edit Custom Sort Order");
    ui->sort_name->setText(fields.at(0));
    ui->sort_name->setEnabled(false);
    ui->label->setText("Name of the Sort you are editing");
    for (int i = 1; i < fields.length(); i++) {
        int index = fieldBoxes.at(i - 1)->findText(fields.at(i));
        fieldBoxes.at(i - 1)->setCurrentIndex(index);
