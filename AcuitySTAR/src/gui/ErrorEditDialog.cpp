    ...
    ui(new Ui::ErrorEditDialog)
{
    ...
    for (it = errors.begin(); it != errors.end(); it++) {
        for (int col = 0; col < (int) headers.size() && col < (int) (*it)->size(); col++) {
            ...
            // Comment out to make all text fields editable, remove comment to make fields not editable.
    ...
    //check if mandatory fields have been filled
        ...
        ui->lableErrorNums->setText(QString::number(numErrors));
...
{
    for (int row = ui->tableWidget->currentRow() + 1; row < ui->tableWidget->rowCount(); row++) {
        for (int j = 0; j < (int) mandatoryList.size(); j++) {
            std::vector<std::string>::iterator it = std::find(headerList.begin(), headerList.end(), mandatoryList[j]);
            int col = it - headerList.begin();
            QTableWidgetItem* item = ui->tableWidget->item(row, col);
...
{
    for (int row = ui->tableWidget->currentRow() - 1; row >= 0; row--) {
        for (int j = mandatoryList.size() - 1; j >= 0; j--) {
            std::vector<std::string>::iterator it = std::find(headerList.begin(), headerList.end(), mandatoryList[j]);
            int col = it - headerList.begin();
            QTableWidgetItem* item = ui->tableWidget->item(row, col);
            if (item->text().compare("") == 0) {
...
{
    if (!initFinished || deinitStarted) { return; }

     QBrush valid(QColor(72, 191, 80));
     QBrush invalid(QColor(255, 0, 0, 100));
